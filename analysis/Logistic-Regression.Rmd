...
Pre-processing has already performed in *HELOC-data-cleaning.Rmd*
...
The BoE QII model uses log(odds) and probability of default as metrics in graphs that enable interpretation. Therefore, it is informative to review these concepts.
...
$$ y_i = \mathbf{Binom}(p_i)$$ $y_i$ is the **Risk** variable and it takes the values 0 or 1. Generally in credit management, **Risk** = 1 signifies default on a loan, while **Risk** = 0 is non-default.
...
For simplicity, consider a logistic regression model based on the first two features.
...
-   $\beta_j < 0 \implies \exp^{\beta_j} < 1$

-   $\beta_j > 0 \implies \exp^{\beta_j} > 1$

In the logistic regression model above, the coefficient for `ExternalRiskEstimate` is -0.105. If the `ExternalRiskEstimate` increases by one unit, then the Odds for default is multiplied by 0.9 ($e^{-0.105}$). This means that, for each unit increase in ExternalRiskEstimate, then the probability of default lowers by 10%.
...
a logistic regression model induced from the HELOC training data, limited to the first two features:
