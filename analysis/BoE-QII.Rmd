...
These answer questions related to individual predictions.  Plots are generated for specific applicants showing feature influence in terms of log(Odds of default) compared to the average log odds across all applicants.  Figure shows plots for two individual applicants.  

as shown in Figure \@ref(fig:keyword)

(ref:keyword) Caption, with optional Markdown links, but no endnotes.

```{r keyword, fig.cap="(ref:keyword)", out.width="100%"}
...
Type 1 explanations are directed at the Developer and Conduct Regulator and allow them to answer questions, such as, "Which features mattered most to individual applicant predictions"?  As the name might imply, explanations are not given in textual terms.  Graphical display facilitates explanation.
